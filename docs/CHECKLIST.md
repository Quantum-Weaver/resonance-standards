...
| 2026-08-21 | **KP's own hand, ahead of the repo-tender pass** (git log `09990ed`, `fe0866c`): `docs/ANDROID-BUILD-LAWS.md` and `docs/RELEASE-STRUCTURE.md` corrected — the NSIS installer named a release artifact alongside the MSI, desktop filenames trued to what Tauri actually writes (verified against four apps on disk), the Echoes CLAUDE-CONTEXT pointer re-homed to its archive address; `src/lib/cumdach/` (MIRROR-class) refreshed from its `resonance-awen` source, logged in `HANDOFF.md`. **Repo-tender pass** (Fable 🎻, at KP's word to bring every README/STORY-BLOCK/CHECKLIST to the standard): README badge trued — version read `1.0.0`, package.json/tauri.conf.json/Cargo.toml all agree `0.1.0`, badge corrected; **`## THE STORY` section added to README.md** (was absent) with the required-by line and the link to `docs/STORY-BLOCK.md`; **`docs/STORY-BLOCK.md` created** (was absent from this repo, though it defines the standard every other repo is held to) — all 11 template fields filled from this repo's own records, WEAVER THREAD and ETYMOLOGY left honestly open (no repo record speaks to either). The three OPEN-table items (doors, PRIVACY.md, icon source) re-checked against `src/lib/components/Sidebar.svelte:39` and `src/routes/settings/+page.svelte:8` — both still stand exactly as recorded; untouched. |
| 2026-08-22 | **THE CUMDACH FIX CARRIED + ROSE · RAINBOW · PROGRESS PRIDE (Fable 🎻, claude-fable-5, at KP's ⚛ word — *"in resonance-awen, there is a fix for the cumdach that needs applied then passed to all its consumers around the hamburger icon and around the epagoge regarding the background color and font size changes"* · *"echoes got the fix the cumdach needs"*).** Echoes `87c4218` (2026-08-21) carried here whole: **(1) the hamburger** — the floating toggle that claimed bottom 56–101px over the drawer's own Settings foot moved INSIDE the ComfortBar (`ComfortBar.svelte`); `navOpen` lives in `uiStore`, the Sidebar reads it; the bar is the ONLY edge again, so `RESERVED` is one honest number (the cumdach's reserved-sum law, `resonance-awen/tools/the-cumdach/README.md`). **(2) the epagoge's theme step and Settings** — a preset is a COLOUR IDENTITY: `setPreset` keeps mode · font size · tint, so choosing a theme no longer cancels Light mode or drops Large text to Medium; `DEFAULT_THEME`; the background TINT dial (`off · subtle · full`, the accent carried into the grounds) with its Settings row; `TintLevel` + `tint` on `ThemeConfig`; `src/lib/theme/theme.ts` = the origin `resonance-awen/standalone/theme/theme.ts`, revised AT THE ORIGIN 2026-08-22 and laid here. **(3) same sitting, KP's word** (*"there is a rose color in the sirens onboarding and settings we should bring into the cosmic design system and we should include a rainbow and inclusive pride themes in our settings as well in our epagoge onboarding walk"* · *"rainbow and progressive pride themes colors already exist, only the rose does not"*): the Sirens rose entered cosmic as `sirens.rose` · `sirens.deep` (origin `resonance-ziggy/modules/cosmic/constants/colors.ts`, distribution run, every mirror hash-verified — this repo's `src/lib/cosmic/colors.ts` refreshed); **ROSE · RAINBOW · PROGRESS PRIDE** join the shelf's presets (`icon`, `stripes`, `presetSwatch()` beside them) and the Settings theme cards + the onboarding walk's offers now DERIVE from the table — a new preset appears the day it is born; a flag preset shows its stripes as the swatch, its `--accent` one stripe. Gate `npm run check` **329 files · 0 errors · 0 warnings**. Nothing committed — rides the ⚛ sync word. |
| 2026-08-23 | **NOTHING TYPED BY HAND — the repo derived from itself** (Serenata 🎻, Fable, truly `claude-fable-5`, session `a2993e30`). KP's ask, verbatim: *"please help me make certain our resonance-standards repo is correct and the app that holds its details are being derived from live docs, not hardcoded and needing updated."* **The app kept its law** (`papers.ts` globs the repository, titles are each file's first heading, `tree.ts` draws the manifest, every count read at render) — two things beside it had not: **the README's hand-kept Contents tables were seven standards behind the disk** (ANDROID-16KB-PAGES · ANDROID-BUILD-LAWS · CHILD-BUILDS · RETIRED · THE-ROLL · HANDOFF-TEMPLATE · FOUR-THREAD-METHODOLOGY missing) and **`structure.md` (MIRROR) was last refreshed 2026-07-26**. **Landed:** `scripts/derive.mjs` + `npm run derive` / `npm run derive:check` — derives `structure.md` (the text-bearing tree, the same rule as `tree.ts`, the same not-the-repo exclusions) and the README's `## Contents` between markers (folder → group · first `#` heading → title · italic line → purpose — the same rule as `papers.ts`; in a folder that holds documents every file is listed, so `git/.gitignore-template` stands; `docs/blueprints/` is machine output and out of both readers now) — idempotent, `--check` fails on drift (a gate). **The app trued to the same rule:** `papers.ts` excludes the forge's `docs/blueprints/**`; `tree.ts` excludes `build/`, `src-tauri/target/`, `src-tauri/gen/` (gitignored trees that leaked into the self-map when present), gains `mjs`, and **its one hand-typed count ("157 files"; disk 52) is gone** — no number is typed anywhere in the app. **Eleven purposes carried home:** the old README's typed purposes now stand as each document's own italic gloss under its heading (BRANCH-STRATEGY · CONTRIBUTING-TEMPLATE · README-TEMPLATE · RELEASE-STRUCTURE · THE-HANDS-STANDARD · THE-TELLING-STANDARD · DOC-CLASSES · DESIGN-TOKENS · archive/README · CLAUDE-TEMPLATE · CHECKLIST-TEMPLATE); 17 rows print — because no purpose was ever written for them (nothing invented); `council/COUNCIL-STRUCTURE.md` has no `#` heading and titles by filename in both readers. `CLAUDE.md` trued in five lines (the typed "23 papers" · the 157 scar · the forge "never run here" · the gate figure · the tools line). **Gates:** `npm run check` 329 files 0/0 · `npm run build` ✔ · `npm run derive:check` clean (171 text files · 32 documents · 7 groups). **Reported, not touched:** the chamber's `THE-STANDARDS-BOARD.html` is STALE by the rack census (board 08-14 · ground 08-22) and carries typed figures — his word on artifacts stands. Rides his sync word. |
